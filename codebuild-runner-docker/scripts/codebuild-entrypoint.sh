...
# Bitbucket runner Docker image version (should match BITBUCKET_RUNNER_VERSION in release.yml)
BITBUCKET_RUNNER_IMAGE="docker-public.packages.atlassian.com/sox/atlassian/bitbucket-pipelines-runner:5.6.0"

# Detect architecture and set appropriate label (add custom label if provided)
ARCH=$(uname -m)
if [ "$ARCH" = "aarch64" ] || [ "$ARCH" = "arm64" ]; then
  BASE_LABELS="linux.arm64,codebuild"
else
  BASE_LABELS="linux,codebuild"
fi

if [ -n "${RUNNER_LABEL:-}" ]; then
  RUNNER_LABELS="${BASE_LABELS},${RUNNER_LABEL}"
else
  RUNNER_LABELS="${BASE_LABELS}"
fi
...
case "$PHASE" in
  pre_build)
    ...
    # Start runner as Docker container
    WORK_DIR="/tmp/runner-work"
    mkdir -p "$WORK_DIR"
    
    RUNNER_CONTAINER_NAME="runner-${RUNNER_UUID//[\{\}]/}"
    echo "$RUNNER_CONTAINER_NAME" > /tmp/runner-container.name
    
    echo "Starting runner Docker container: ${RUNNER_CONTAINER_NAME}"
    docker run -d \
      --name "${RUNNER_CONTAINER_NAME}" \
      -v /tmp:/tmp \
      -v /var/run/docker.sock:/var/run/docker.sock \
      -v /var/lib/docker/containers:/var/lib/docker/containers:ro \
      -v /runner/scripts/logback-console.xml:/opt/atlassian/runner/logback-console.xml:ro \
      -e ACCOUNT_UUID="${WORKSPACE_UUID}" \
      -e REPOSITORY_UUID="${REPO_UUID}" \
      -e RUNNER_UUID="${RUNNER_UUID}" \
      -e OAUTH_CLIENT_ID="${RUNNER_OAUTH_CLIENT_ID}" \
      -e OAUTH_CLIENT_SECRET="${RUNNER_OAUTH_CLIENT_SECRET}" \
      -e RUNTIME_PREREQUISITES_ENABLED=true \
      -e WORKING_DIRECTORY="${WORK_DIR}" \
      -e JAVA_OPTS="-Dlogback.configurationFile=/opt/atlassian/runner/logback-console.xml" \
      "${BITBUCKET_RUNNER_IMAGE}"
    
    echo "Container environment:"
    echo "  ACCOUNT_UUID: ${WORKSPACE_UUID}"
    echo "  REPOSITORY_UUID: ${REPO_UUID}"
    echo "  RUNNER_UUID: ${RUNNER_UUID}"
    echo "  WORKING_DIRECTORY: ${WORK_DIR}"
    
    echo "Runner container started: ${RUNNER_CONTAINER_NAME}"
    
    # Wait for runner to become ONLINE
    export RUNNER_UUID
    export RUNNER_CONTAINER_NAME
  ...
  build)
    ...
    # Read runner container name if exists
    if [ -f /tmp/runner-container.name ]; then
      export RUNNER_CONTAINER_NAME=$(cat /tmp/runner-container.name)
    fi
  ...
  post_build)
    ...
    if [ -f /tmp/runner-container.name ]; then
      RUNNER_CONTAINER_NAME=$(cat /tmp/runner-container.name)
      if docker ps -q -f name="${RUNNER_CONTAINER_NAME}" | grep -q .; then
        echo "Stopping runner container: ${RUNNER_CONTAINER_NAME}"
        docker stop "${RUNNER_CONTAINER_NAME}" 2>/dev/null || true
        docker rm "${RUNNER_CONTAINER_NAME}" 2>/dev/null || true
      fi
    fi
